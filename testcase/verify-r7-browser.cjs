...
(async () => {
  ...
  try {
    ...
    const filterWidths = await page.locator('#trafficFilter, #projectFilter').evaluateAll((nodes) => Object.fromEntries(nodes.map((node) => [node.id, Math.round(node.getBoundingClientRect().width)])));
    assert.ok(filterWidths.trafficFilter < filterWidths.projectFilter);
