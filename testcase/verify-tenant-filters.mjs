...
try {
  ...
  assert.equal(await lockStateCell.locator('.threshold-custom-value').textContent(), '关闭');
  await reset(page);
  ...
  assert.equal(await lockStateCell.locator('.threshold-custom-value').textContent(), '开启');
  await reset(page);
